...
const { store } = await createStoreBridgeBrowser<State>({
  ...
  pageId: "test-page-1771076264924",
  storeKey: 'main',
  meta: {
    id: "test-page-1771076264924",
    title: "test-page-1771076264924",
    store: {
      stateSchema,
      actions: [
        {
          type: 'test-page-1771076264924.setData',
  ...
  createState: (set, get) => ({
    ...
    items: [],
    dispatch: (action) => {
      if (action.type === 'test-page-1771076264924.setData') {
  ...
  return (
    <div className="p-6">
      <Card className="max-w-2xl">
        <CardHeader>
          <CardTitle>test-page-1771076264924</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div className="space-y-2">
            {items.length === 0 ? (
              <div className="text-sm text-muted-foreground">No data yet. Use StoreBridge SDK to dispatch 'test-page-1771076264924.setData'.</div>
            ) : (
          ...
          <Button variant="secondary" onClick={() => store.getState().dispatch({ type: 'test-page-1771076264924.setData', payload: { updatedAt: Date.now(), items: [{ id: crypto.randomUUID?.() ?? String(Math.random()), text: 'hello from browser' }] } })}>
