...
# 查看页面状态/动作信息
agent-stage run inspect counter

# 调用动作
agent-stage run exec counter someAction '{"x":1}'
```

## 文件模式 vs 在线模式

1. 文件模式：直接写 `store.json`，页面下次加载可见。  
2. 在线模式：要求 `agent-stage serve <pageId>` 已运行且页面已打开；可立即生效。

## 故障排查

1. `Runtime is not running`：先执行 `agent-stage serve <pageId>`。  
